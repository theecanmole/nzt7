...
mtext(side=1,cex=0.7,line=-1.3,"Data: https://niwa.co.nz/climate-and-weather/annual/annual-climate-summary-2025 \nhttps://www.niwa.co.nz/sites/niwa.co.nz/files/NZT7_Adjusted_Annual_TMean2018_Web-updated-jan-2019.xlsx")
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(t7data[["Year"]],t7data[["Anomaly"]],ylim=c(-1.5,1.45),xlim=c(1905,2025),tck=0.01,axes=FALSE,ann=FALSE, type="l",col="1",lwd=1,las=1)
...
par(mar=c(2.7,2.7,1,1)+0.1)
plot(t7timeseries,ylim=c(-1.5,1.45),xlim=c(1905,2024),tck=0.01,axes=FALSE,ann=FALSE, type="l",col="1",lwd=1,las=1)
axis(side=1, tck=0.01, las=0,tick=TRUE)
axis(side=2, tck=0.01, las=0,tick=TRUE,las=1)
box()
lines(t7timeseries,col="1",lwd=1)
points(t7timeseries,col="#000099",pch=19)
lines(lowess(t7timeseries,f=0.25),lwd=3,col="#CC0000")
mtext(side=1,cex=0.7,line=-1.1,"Data: NIWA Seven-station series temperature data\n https://www.niwa.co.nz/sites/niwa.co.nz/files/NZT7_Adjusted_Annual_TMean2018_Web-updated-jan-2019.xlsx")
mtext(side=3,cex=1.7, line=-4,expression(paste("New Zealand Mean Land Surface \nTemperature Anomalies 1909 - 2024")) )
mtext(side=2,cex=1, line=-1.3,"Temperature anomaly C vs 1981-2010 mean")
legend(1910, 1,bty='n',bg="white", cex = 0.9, c(paste("Mean", c("annual anomaly", "lowess smoothed anomaly f = 0.25"))),pch=c(19,NA),lty=c(1,1),lwd=c(1,3),col=c("#000099","#CC0000"))
mtext(side=4,cex=0.75, line=0.05,R.version.string)
abline(h=0,col="darkgray")
dev.off()
